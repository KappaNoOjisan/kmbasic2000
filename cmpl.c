...
#include "main.h"
void isStr(void) __naked {
	/* 
	char j=g_vardesc[a];
	if (j==VAR_NULL)
		return 2;
	else if ((j&VAR_INTT)==0)
		return 1;
	else
		return 0;
	*/
...
void isAry(void) __naked {
	/*
	char j=g_vardesc[i];
	if (j==VAR_NULL)
		return 2;
	else if((j&VAR_ARYT)==0)
		return 0;
	else if(k<=g_varlimit[i])
		return 3;
	else
		return 1;
	*/
...
char compileStr(){
	...
	if (b=='"') {
		...
		j=(int)(b-'A');
		// type check
		copyByte(0x21);			// LD HL,#&g_vardesc[j]
		copyInt((int)&g_vardesc[j]);
		copyByte(0x7e);			// LD A,(HL)
		copyByte(0xe6);			// AND #VAR_INTT
		copyByte(VAR_INTT);
		copyCode("\x28\x09\x3e",3);	// JR Z, skip
		object+=3;
		copyByte(ERR_TYPEOF);		// LD A,#ERR_TYPEOF
		copyCode("\xf5\x33\xcd",3);	// PUSH AF; INC SP
		object+=3;			// CALL _errorAndEnd
		copyInt((int)errorAndEnd);	// INC SP;
		copyCode("\x33\xc9",2);		// RET
		object+=2;			//skip:
		// let
...
char compileIntSub(){
	...
	if (b=='(') {
		...
		copyByte(0xe6);			// AND #VAR_INTT
		copyByte(VAR_INTT);
		copyCode("\x20\x09\x3e",3);	// JR NZ,skip
		object+=3;
		copyByte(ERR_TYPEOF);		// LD A,#ERR_TYPEOF
		copyCode("\xf5\x33\xcd",3);	// PUSH AF; INC SP
		object+=3;			// CALL _errorAndEnd
...
char compileLet(){
	...
	if (b=='$') {
		...
		object+=5;
		// type set
		if ( g_vardesc[j] == VAR_NULL ) {
			copyByte(0x21); // LD HL,#&g_vardesc[j]; LD (HL),#(VAR_USED);
			copyInt((int)&g_vardesc[j]);
			copyByte(0x36);
			copyByte(VAR_USED);
		}
		...
		// type set
		if ( g_vardesc[j] == VAR_NULL ) {
			copyByte(0x21); // LD HL,#&g_vardesc[j]; LD (HL),#(VAR_USED|VAR_INTT);
			copyInt((int)&g_vardesc[j]);
			copyByte(0x36);
			copyByte(VAR_USED|VAR_INTT);
		}
	}
	return ERR_NOTHIN;
}
...
char compileDim(){
	...
	while(1){
		...
		j=(b-'A');
		// type set
		copyByte(0x21); // LD HL,#&g_vardesc[j]; LD (HL),#(VAR_USED|VAR_INTT|VAR_ARYT);
		copyInt((int)&g_vardesc[j]);
		copyByte(0x36);
		copyByte(VAR_USED|VAR_INTT|VAR_ARYT);
		// set limit		
		variableAddress=(int)(&g_variables[j*2]);
		b=compileInt();
		if (b) return b;
